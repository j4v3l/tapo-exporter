...
class TapoMetrics:
    ...
    async def update_metrics(self, device: P110Device, update_usage: bool = True) -> None:
        ...
        try:
            ...
            device_name = device_info.nickname if hasattr(device_info, 'nickname') else device.name
            device_type = device_info.model.lower() if hasattr(device_info, 'model') else "p110"
            ...
            voltage = abs(
                power_info.voltage if hasattr(power_info, 'voltage') else 0
            )
            ...
            if update_usage:
                ...
                if usage_info is None:
                    ...
                    today_energy = abs(
                        usage_info.today_energy if hasattr(usage_info, 'today_energy') else 0
                    )
                    month_energy = abs(
                        usage_info.month_energy if hasattr(usage_info, 'month_energy') else 0
                    )
                    power_saved = abs(
                        usage_info.power_saved if hasattr(usage_info, 'power_saved') else 0
                    )
                    today_runtime = abs(
                        usage_info.today_runtime if hasattr(usage_info, 'today_runtime') else 0
                    )
                    month_runtime = abs(
                        usage_info.month_runtime if hasattr(usage_info, 'month_runtime') else 0
                    )
                    
                    # Log usage info if any values are non-zero
                    if any([today_energy, month_energy, power_saved, today_runtime, month_runtime]):
                        logger.info(
                    ...
                    self.today_energy_wh.labels(
                        device_name, device_type
                    ).set(today_energy)
                    
                    self.month_energy_wh.labels(
                        device_name, device_type
                    ).set(month_energy)
                    
                    self.power_saved_wh.labels(
                        device_name, device_type
                    ).set(power_saved)
                    
                    self.today_runtime_minutes.labels(
                        device_name, device_type
                    ).set(today_runtime)
                    
                    self.month_runtime_minutes.labels(
                        device_name, device_type
                    ).set(month_runtime)
                    
                    # Update protection status
                    power_protection = int(
                        usage_info.power_protection if hasattr(usage_info, 'power_protection') else False
                    )
                    self.power_protection_status.labels(
                        device_name, device_type
                    ).set(power_protection)
                    
                    overcurrent = int(
                        usage_info.overcurrent_protection if hasattr(usage_info, 'overcurrent_protection') else False
                    )
                    self.overcurrent_status.labels(
                        device_name, device_type
                    ).set(overcurrent)
                    
                    overheat = int(
                        usage_info.overheat_protection if hasattr(usage_info, 'overheat_protection') else False
                    )
                    self.overheat_status.labels(
                        device_name, device_type
                    ).set(overheat)
                    
                    # Update signal info
                    signal = usage_info.signal_strength if hasattr(usage_info, 'signal_strength') else 0
