...
async def test_update_metrics(metrics):
    ...
    def get_value(metric):
        samples = [s for s in metric.collect()[0].samples if s.labels == labels]
...
async def test_metrics_update_error():
    ...
    with patch("tapo_exporter.metrics.logger") as mock_logger:
        ...
        expected_msg = "Error updating metrics for device 192.168.1.100: Test error"
...
async def test_metrics_missing_device_info():
    ...
    with patch("tapo_exporter.metrics.logger") as mock_logger:
        ...
        expected_msg = (
            "Failed to get device info for Test Device. " "Skipping metrics update."
        )
...
async def test_metrics_missing_power_info():
    ...
    mock_device.get_device_info = AsyncMock(
        return_value={"nickname": "Test Device", "model": "P110"}
    )
    ...
    with patch("tapo_exporter.metrics.logger") as mock_logger:
        ...
        # Verify warning was logged
        expected_msg = (
            "Failed to get power info for Test Device. " "Skipping metrics update."
        )
        mock_logger.warning.assert_called_once_with(expected_msg)
...
async def test_metrics_missing_usage_info(metrics):
    ...
    mock_device.get_current_power = AsyncMock(
        return_value={"current_power": 100, "voltage": 120, "current": 0.5}
    )
